...
class JointVAE(nn.Module):
    def __init__(self, input_size_A, input_size_B, input_size_C, input_size, seq_len):
        ...
        self.input_size_A_low = input_size_A // 2
        self.input_size_B_low = input_size_B // 2
        self.input_size_C_low = input_size_C // 2

        self.en_fc_A = nn.Linear(in_features=self.input_size_A_low, out_features=self.input_size_low)
        ...
        self.decoder_linear_A = nn.Sequential(
            nn.Linear(in_features=input_size * seq_len, out_features=self.input_size_A_low)
        )
    ...
    def encode(self, x_A, x_B, x_C):
        x_A = x_A.view(x_A.shape[0], 2, self.input_size_A_low)
    ...
    def decode(self, z_l):
        ...
        x_A_reconstructed = x_A_reconstructed.view(x_A_reconstructed.shape[0], 2, self.input_size * self.seq_len)
