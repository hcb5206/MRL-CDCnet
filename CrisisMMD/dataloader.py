...
def dataloader(batch_size, task):
    if task == '1':
        filename = 'data/data_crisismmd_task1.pkl'
        num_class = 2
    else:
        filename = 'data/data_crisismmd_task2.pkl'
        num_class = 5
    ...
    print(label_train)
    counts_dict, counts_dict_por = count_samples_per_class(label_train, num_class)
    print(counts_dict)
    ...
    print(label_train.shape, label_encoder_train.shape, word_train_1.shape, word_train_2.shape, word_train_3.shape,
          word_train_4.shape, word_train_all.shape, img_train.shape)
    ...
    print(label_val)
    counts_dict_val, counts_dict_por_val = count_samples_per_class(label_val, num_class)
    ...
    print(label_test)
    counts_dict_test, counts_dict_por_test = count_samples_per_class(label_test, num_class)
    ...
    print(label_test.shape, word_test_1.shape, word_test_2.shape, word_test_3.shape,
          word_test_4.shape, word_test_all.shape, img_test.shape)

    train_dataset = TensorDataset(img_train, word_train_1, word_train_2, word_train_3,
                                  word_train_4, word_train_all, label_train, label_encoder_train)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    val_dataset = TensorDataset(img_val, word_val_1, word_val_2, word_val_3, word_val_4, word_val_all,
                                label_val)

    val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)

    return train_dataloader, val_dataloader, img_test, word_test_1, word_test_2, word_test_3, \
           word_test_4, word_test_all, label_test
