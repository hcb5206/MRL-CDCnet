...
class OutputLayer(nn.Module):
    def __init__(self, input_size, seq_len, hidden_size, num_heads, dropout, k, label_size, classer,
                 input_size_img, input_size_word, p):
        ...
        self.jointVAE = JointVAE(input_size_A=input_size_img, input_size_B=input_size_word, input_size=input_size,
                                 seq_len=seq_len)
        ...
        self.normBN_img = nn.BatchNorm1d(self.input_size_img, affine=True)
        self.normBN_diff_img = nn.BatchNorm1d(self.input_size_img, affine=True)

        self.fc_text = nn.Linear(in_features=self.input_size_word // 4, out_features=(input_size * seq_len) // 4)
        self.normBN_text = nn.BatchNorm1d(input_size * seq_len, affine=True)

        self.diff_fc_text = nn.Linear(in_features=self.input_size_word // 4, out_features=(input_size * seq_len) // 4)
        self.normBN_diff_text = nn.BatchNorm1d(input_size * seq_len, affine=True)
    ...
    def forward(self, vision, text_1, text_2, text_3, text_4, text_all):
        simi, simi_vision, simi_text, mu, logvar = self.jointVAE(vision, text_all)
        ...
        text = text.view(text.shape[0], 4, self.input_size_word // 4)
        text = self.fc_text(text)
        text = text.view(text.shape[0], -1)
        text = self.normBN_text(text)

        diff_text = diff_text.view(diff_text.shape[0], 4, self.input_size_word // 4)
        diff_text = self.diff_fc_text(diff_text)
        diff_text = diff_text.view(diff_text.shape[0], -1)
        diff_text = self.normBN_diff_text(diff_text)

        diff = self.Simi_Diff_Fusion_diff(diff_vision, diff_text)
        diff = self.DiffFilter(diff)
        # diff = torch.cat((diff_vision, diff_text), dim=1)
        # diff = diff_vision * diff_text
        # diff = diff1 + diff2
        comple_out = self.ComplementarityAttention(vision, text, diff)
        fusion_out = self.FusionLayerAttention(simi, diff, comple_out)
        ...
        return out, comple_out, simi_vision, simi_text, mu, logvar
...
class CustomLoss(nn.Module):
    ...
    def forward(self, fusion_out, comple_out, labels, labels_encoder, x_A, x_A_reconstructed, x_B, x_B_reconstructed,
        ...
        comple_loss = tube_A + tube_B + kl_loss + ce_loss + st_all
        return comple_loss
