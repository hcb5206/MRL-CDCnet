...
class ComplementarityAttention(nn.Module):
    ...
    def forward(self, vision, text, diff):
        ...
        att_vision = self.cross_att_vision(vision, text, diff).unsqueeze(1)
        att_text = self.cross_att_text(text, vision, diff).unsqueeze(1)
        att_in = torch.cat((att_vision, att_text), dim=1)
