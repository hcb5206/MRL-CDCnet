...
class OutputLayer(nn.Module):
    def __init__(self, input_size, seq_len, hidden_size, num_heads, dropout, k, label_size, classer,
                 input_size_img, input_size_word, input_size_audio, p):
        ...
        self.normBN_text = nn.BatchNorm1d(input_size_word, affine=True)
        self.normBN_diff_text = nn.BatchNorm1d(input_size_word, affine=True)

        self.fc_img = nn.Sequential(nn.Linear(in_features=input_size_img, out_features=input_size * seq_len),
                                    nn.BatchNorm1d(input_size * seq_len, affine=True))

        self.diff_fc_img = nn.Sequential(nn.Linear(in_features=input_size_img, out_features=input_size * seq_len),
                                         nn.BatchNorm1d(input_size * seq_len, affine=True))
    ...
    def forward(self, vision, text, audio):
        simi, simi_vision, simi_text, simi_audio, mu, logvar = self.jointVAE(vision, text, audio)
        ...
        diff = self.Simi_Diff_Fusion_diff(diff_vision, diff_text, diff_audio)
        ...
        if self.classer == 'parm':
            out = self.linear(fusion_out).squeeze()
        else:
...
class CustomLoss(nn.Module):
    def __init__(self, l1=0.5, l2=1.5, alpha=1.0, beta=50000000.0, loss='MSE', eps=1e-8):
