...
class JointVAE(nn.Module):
    ...
    def decode(self, z_l):
        ...
        x_A_reconstructed = x_A_reconstructed.view(x_A_reconstructed.shape[0], self.seq_len, self.input_size * 2)
        x_A_reconstructed = self.decoder_linear_A(x_A_reconstructed)
        x_B_reconstructed = self.decoder_B(z_l)
        x_B_reconstructed = x_B_reconstructed.view(x_B_reconstructed.shape[0], self.seq_len, self.input_size * 2)
        x_B_reconstructed = self.decoder_linear_B(x_B_reconstructed)
        x_C_reconstructed = self.decoder_C(z_l)
        x_C_reconstructed = x_C_reconstructed.view(x_C_reconstructed.shape[0], self.seq_len, self.input_size * 2)
