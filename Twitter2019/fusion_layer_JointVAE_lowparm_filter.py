...
class OutputLayer(nn.Module):
    def __init__(self, input_size, seq_len, hidden_size, num_heads, dropout, k, label_size, classer, p,
                 dif=True):
        ...
        self.jointVAE = JointVAE(input_size=input_size, seq_len=seq_len)

        self.normBNa = nn.BatchNorm1d(input_size * seq_len, affine=True)
        self.normBNb = nn.BatchNorm1d(input_size * seq_len, affine=True)
        self.normBNc = nn.BatchNorm1d(input_size * seq_len, affine=True)
        self.normBNd = nn.BatchNorm1d(input_size * seq_len, affine=True)
        ...
        self.normBN_diff_word = nn.BatchNorm1d(input_size * seq_len, affine=True)
    ...
    def forward(self, vision, text_1, text_2, text_3, text_4, text_all, audio):
