...
class OutputLayer(nn.Module):
    def __init__(self, input_size, seq_len, hidden_size, num_heads, dropout, k, label_size, classer,
        ...
        self.normBN_img = nn.BatchNorm1d(input_size_img, affine=True)
        self.normBN_diff_img = nn.BatchNorm1d(input_size_img, affine=True)

        self.normBN_word = nn.BatchNorm1d(input_size_word, affine=True)
        self.normBN_diff_word = nn.BatchNorm1d(input_size_word, affine=True)

        self.ComplementarityAttention = ComplementarityAttention(input_size=input_size, seq_len=seq_len,
                                                                 hidden_size=hidden_size,
                                                                 num_heads=num_heads, dropout=dropout, k=k)
    ...
    def forward(self, vision, text):
        simi, simi_vision, simi_text, mu, logvar = self.jointVAE(vision, text)

        simi = self.normBN_simi(simi)

        diff_vision = torch.sub(vision, simi_vision)
        diff_text = torch.sub(text, simi_text)

        vision = self.normBN_img(vision)
        diff_vision = self.normBN_diff_img(diff_vision)
        text = self.normBN_word(text)
        diff_text = self.normBN_diff_word(diff_text)

        diff_first = self.Simi_Diff_Fusion_diff(diff_vision, diff_text)
